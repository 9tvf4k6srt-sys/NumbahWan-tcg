...
import { ACHIEVEMENTS, GAME_CACHE_KEYS } from './gamification-types'
export type {
  Prediction,
  Achievement,
  UserAchievement,
...
export async function resolvePredictions(
  ...
  for (const pred of predictions) {
    ...
    const won = (pred.direction === 'up' && priceWentUp) || (pred.direction === 'down' && !priceWentUp)
...
export async function updateLeaderboard(cache: KVNamespace, predictions: Prediction[]): Promise<void> {
