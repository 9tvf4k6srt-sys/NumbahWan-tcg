import type { ActionType, GuideAction, GuideRequest, GuideResponse, Message } from './ai-guide-types'
...
function buildSystemPrompt(language: string, currentPage?: string, _userContext?: Record<string, any>): string {
...
export async function chat(request: GuideRequest, apiKey: string, baseUrl: string): Promise<GuideResponse> {
  const { message, conversationHistory = [], language = 'en', currentPage, userContext } = request

  // Build messages array with full context
  const messages: Message[] = [{ role: 'system', content: buildSystemPrompt(language, currentPage, userContext) }]
