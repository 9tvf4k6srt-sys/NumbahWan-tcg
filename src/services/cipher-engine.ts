...
import type { CipherHint, CipherType, CipherVault, CipherWinner } from './cipher-types'
import { CACHE_KEYS, DIFFICULTY_SETTINGS } from './cipher-types'
...
function _reverseAndCaesar(text: string, shift: number): string {
  ...
  return [
    {
      ...
      flavorLore:
        'An ancient guild message was found in the Perion archives, encoded by a simple shift cipher. The warriors who wrote it trusted no one.',
    ...
    },
    {
      ...
      flavorLore:
        'Deep in the guild database, a message was encoded multiple times. Each layer of encoding hides the truth further.',
    ...
    },
    {
      ...
      flavorLore:
        "The guild's inner circle communicated using a scrambled alphabet. Only those who can reconstruct the mapping will read their secrets.",
    ...
    },
    {
      ...
      flavorLore:
        'Protected by four layers of encryption, this message was sealed by the guild founders. Caesar, reverse, Base64, and Caesar again. Only the worthy shall break all four seals.',
      encrypt: () => {
    ...
    },
    {
      ...
      flavorLore:
        "Hidden for generations. Reversed, shifted, encoded, shifted again, and reversed once more. The guild's ultimate secret rewards only the most persistent decoder.",
      encrypt: () => {
      ...
      hints: [
        ...
        {
          text: 'Undo from outside in: reverse → un-Caesar(8) → un-Base64(×2) → un-ROT13 → reverse.',
          revealAfterDays: 45,
        },
      ],
    },
...
export async function createVault(templateIndex?: number): Promise<CipherVault> {
  ...
  const idx =
    templateIndex !== undefined ? templateIndex % templates.length : Math.floor(Math.random() * templates.length)
...
export async function recordAttempt(cache: KVNamespace, wallet: string, correct: boolean): Promise<void> {
  ...
  const data: { attempts: number[]; cooldownUntil: number } = raw ? JSON.parse(raw) : { attempts: [], cooldownUntil: 0 }
