...
function pcp(endpoint: string, data: any) {
  ...
  return {
    _pcp: { version: PCP_VERSION, endpoint, tokens: Math.ceil(json.length / 4), generated_at: Date.now() },
    ...data,
  }
}
...
async function kvGet(kv: KVNamespace | undefined, key: string, fallback: any = null) {
  ...
  try {
    const raw = await kv.get(`pcp:${key}`)
...
async function kvSet(kv: KVNamespace | undefined, key: string, value: any, ttl = 86400 * 30) {
  if (!kv) return false
  try {
    await kv.put(`pcp:${key}`, JSON.stringify(value), { expirationTtl: ttl })
    return true
  } catch {
    return false
  }
...
function healthScore() {
  return r?.summary?.healthScore || 0
}
function healthGrade() {
  return r?.summary?.grade || '?'
}
function weakest(n = 3) {
  return Object.entries(r?.modules || {})
    .map(([name, m]: [string, any]) => ({ name, score: m.score }))
  ...
  c.json(
    pcp('brief', {
      project: {
        ...
        description: 'Browser-based TCG + guild warfare. Hono + Cloudflare Workers. 119 pages, zero frameworks.',
      },
      stack: {
    ...
    pcp('rules', {
      ...
      categories: {
        design: [
          {
            id: 'TWO_BRANDS',
            rule: 'NW (#ff6b00) and KINTSUGI (#c9a84c) never cross-contaminate',
            severity: 'critical',
          },
          {
            id: 'MOBILE_FIRST',
            rule: 'Owner reviews on iPhone 375px. Test mobile before desktop.',
            severity: 'critical',
          },
...
router.get('/context', (c) => {
  const modules = Object.entries(r?.modules || {}).map(([name, mod]: [string, any]) => ({
    ...
    top_issues: (mod.issues || [])
      .slice(0, 3)
      .map((i: any) => ({ id: i.id, severity: i.severity, message: i.message })),
  }))
  return c.json(
    pcp('context', {
      identity: {
        ...
        description:
          'Browser-based TCG + guild warfare. Two brands (NW + KINTSUGI), trilingual, 119 pages, zero frameworks.',
      },
      stack: {
        runtime: 'Cloudflare Workers',
        framework: 'Hono',
      ...
      architecture: {
        ...
        modules: 30,
        structure: {
          'src/routes/*.ts': '30 route modules',
          'src/routes/agent.ts': 'PCP reference implementation',
          'public/': '119 HTML pages + static assets',
          'sentinel.cjs': 'NW-GUARDIAN v3.0 (codebase health)',
          'mycelium.cjs': 'Project memory core',
          'bin/agent-brief.cjs': 'CLI agent brief tool',
        },
      },
      health: { score: healthScore(), grade: healthGrade(), modules },
      conventions: {
        style: 'Vanilla JS, no frameworks, single-file HTML pages',
        naming: 'kebab-case files, nw-* prefix for shared scripts',
        patterns: ['static HTML + JS hydration', 'Hono API routes', 'KV-backed persistence', 'sentinel health scoring'],
      },
      owner: {
        preferences: { mobile_first: true, two_brands: true, no_emoji_icons: true },
        review_device: 'iPhone 375px, iOS Safari',
...
router.get('/tasks', async (c) => {
  ...
  if (criticals.length > 0)
    suggestions.push({
      priority: 'high',
      type: 'fix',
      desc: `Fix ${criticals.length} critical issue(s)`,
      items: criticals.slice(0, 5).map((i: any) => i.message || i.id),
    })
  const weak = Object.entries(r?.modules || {})
    .filter(([, m]: [string, any]) => m.score < 50)
    .map(([n, m]: [string, any]) => `${n}(${m.score})`)
  if (weak.length > 0)
    suggestions.push({ priority: 'medium', type: 'improve', desc: `Improve weak modules: ${weak.join(', ')}` })
  ...
  return c.json(
    pcp('tasks', {
      ...
      recently_completed: completed.slice(0, 5),
      suggestions,
    }),
  )
})
...
router.post('/tasks', async (c) => {
  ...
  if (action === 'add') {
    ...
    await appendLog(kv, { type: 'task_added', task: task.id, desc: task.desc })
  ...
  if (action === 'update' && body.id) {
    ...
    await kvSet(kv, 'tasks', tasks)
    await appendLog(kv, { type: 'task_updated', task: body.id, status: body.status })
    return c.json({ ok: true, task: tasks[idx] })
  }
  if (action === 'clear_completed') {
    const remaining = tasks.filter((t: any) => t.status !== 'completed')
    await kvSet(kv, 'tasks', remaining)
    return c.json({ ok: true, cleared: tasks.length - remaining.length, remaining: remaining.length })
  }
...
router.get('/memory', async (c) => {
  ...
  return c.json(
    pcp('memory', {
      learnings: memory.learnings.slice(0, 20),
      decisions: memory.decisions.slice(0, 20),
      blockers: memory.blockers.filter((b: any) => !b.resolved),
      sessions: memory.sessions.slice(0, 10),
      stats: {
        total_learnings: memory.learnings.length,
        total_decisions: memory.decisions.length,
        active_blockers: memory.blockers.filter((b: any) => !b.resolved).length,
...
router.post('/memory', async (c) => {
  ...
  if (type === 'learning') {
    ...
    if (memory.learnings.length > 100) memory.learnings.length = 100
    await kvSet(kv, 'memory', memory)
    await appendLog(kv, { type: 'learning_stored', area: body.area })
  ...
  if (type === 'decision') {
    ...
    if (memory.decisions.length > 50) memory.decisions.length = 50
    await kvSet(kv, 'memory', memory)
    await appendLog(kv, { type: 'decision_stored', context: body.context })
    return c.json({ ok: true, entry })
  }
  if (type === 'blocker') {
    const entry = { id: `b-${Date.now()}`, desc: body.desc, area: body.area, resolved: false, ts: Date.now() }
    memory.blockers.unshift(entry)
    await kvSet(kv, 'memory', memory)
    await appendLog(kv, { type: 'blocker_added', area: body.area })
    return c.json({ ok: true, entry })
  }
  ...
  if (type === 'session_start') {
    ...
    if (memory.sessions.length > 30) memory.sessions.length = 30
    await kvSet(kv, 'memory', memory)
    await appendLog(kv, { type: 'session_started', agent: body.agent })
  ...
  return c.json(
    { ok: false, error: 'Invalid type. Use: learning, decision, blocker, resolve_blocker, session_start, session_end' },
    400,
  )
})
...
router.post('/onboard', async (c) => {
  ...
  if (memory.sessions.length > 30) memory.sessions.length = 30
  await kvSet(kv, 'memory', memory)
  await appendLog(kv, { type: 'session_started', agent: agentName, goals })
  const tasks = await kvGet(kv, 'tasks', [])
  const pending = tasks.filter((t: any) => t.status === 'pending')
  ...
  return c.json(
    pcp('onboard', {
      ...
      project: {
        name: 'NumbahWan TCG',
        repo: '9tvf4k6srt-sys/NumbahWan-tcg',
        stack: 'Hono + Cloudflare Workers + Vanilla JS',
        health: { score: healthScore(), grade: healthGrade(), critical: r?.summary?.critical || 0 },
      },
      rules: [
        'TWO BRANDS: NW (#ff6b00) + KINTSUGI (#c9a84c) — never cross-contaminate',
        'MOBILE FIRST: Owner reviews on iPhone 375px',
        'NO EMOJI ICONS: Professional SVG/WebP only',
        'I18N REQUIRED: EN/ZH/TH on NW, EN/ZH/JP on KINTSUGI',
        'COMMIT AFTER EVERY CHANGE, PR AFTER EVERY COMMIT',
      ],
...
router.get('/pulse', async (c) => {
  ...
  return c.json({
    status: unacked > 0 ? 'alert' : pending > 5 ? 'busy' : score < 50 ? 'degraded' : 'ok',
    score,
    pending,
    alerts: unacked,
    ts: Date.now(),
...
router.get('/status', async (c) => {
  ...
  const [tasks, memory, alerts, log] = await Promise.all([
    kvGet(kv, 'tasks', []),
    kvGet(kv, 'memory', { learnings: [], decisions: [], blockers: [], sessions: [] }),
    kvGet(kv, 'alerts', []),
    kvGet(kv, 'log', []),
  ])
  ...
  return c.json(
    pcp('status', {
      ...
      blockers: activeBlockers.slice(0, 5),
      alerts: unackedAlerts.slice(0, 5),
      memory: {
        learnings: memory.learnings.length,
        decisions: memory.decisions.length,
        recent_learning: memory.learnings[0],
        recent_decision: memory.decisions[0],
      },
      last_session: memory.sessions[0]
        ? {
            agent: memory.sessions[0].agent,
            started_at: memory.sessions[0].started_at,
            status: memory.sessions[0].status,
          }
        : null,
...
router.get('/log', async (c) => {
  const kv = c.env?.MARKET_CACHE
  const log = await kvGet(kv, 'log', [])
  return c.json(pcp('log', { entries: log.slice(0, parseInt(c.req.query('limit') || '50', 10)), total: log.length }))
})
...
router.get('/alerts', async (c) => {
  ...
  return c.json(
    pcp('alerts', {
      unacknowledged: alerts.filter((a: any) => !a.acknowledged),
...
router.post('/alerts/ack', async (c) => {
  ...
  const alerts = await kvGet(kv, 'alerts', [])
  if (body.id) {
    const a = alerts.find((a: any) => a.id === body.id)
    if (a) {
      a.acknowledged = true
      a.acked_at = Date.now()
    }
...
const COMMANDS: Record<string, { desc: string; run: string }> = {
  ...
  'context.refresh': {
    desc: 'Regen sentinel report',
    run: 'node sentinel.cjs --json > public/static/data/sentinel-report.json',
  },
}
...
router.post('/actions', async (c) => {
  ...
  if (!command)
    return c.json(
      pcp('actions', {
        available: Object.entries(COMMANDS).map(([name, cmd]) => ({ name, description: cmd.desc, command: cmd.run })),
        usage: 'POST /api/pcp/actions { "command": "health.check" }',
      }),
    )
  ...
  if (command === 'health.check')
    return c.json({
      ok: true,
      command,
      result: { score: healthScore(), grade: healthGrade(), issues: r?.issues?.length },
      modules: Object.entries(r?.modules || {}).reduce((acc: any, [n, m]: [string, any]) => {
        acc[n] = { score: m.score, grade: m.grade }
        return acc
      }, {}),
      note: 'Build-time cached. Run shell command for live.',
    })
  return c.json({
    ok: true,
    command,
    ...
    run: cmd.run,
    hint: 'Execute the "run" command in your shell.',
  })
})
...
router.post('/webhooks/github', async (c) => {
  ...
  try {
    ...
    if (event === 'push') {
      ...
      entry.branch = branch
      entry.commits = commits.length
      entry.pusher = body.pusher?.name
      if (branch === 'main') {
        const tasks = await kvGet(kv, 'tasks', [])
        tasks.push({
          id: `t-gh-${Date.now()}`,
          desc: `Review main push: ${commits.length} commit(s) by ${body.pusher?.name}`,
          priority: 'medium',
          status: 'pending',
          created_at: Date.now(),
    ...
    if (event === 'pull_request') {
      ...
      entry.pr = { number: pr.number, title: pr.title }
    ...
    if (event === 'issues') {
      ...
      entry.action = body.action
      entry.issue = { number: issue.number, title: issue.title }
      if (body.action === 'opened') {
        const tasks = await kvGet(kv, 'tasks', [])
        tasks.push({
          id: `t-iss-${Date.now()}`,
          desc: `Triage issue #${issue.number}: ${issue.title}`,
          priority: 'medium',
          status: 'pending',
          created_at: Date.now(),
          source: 'github-webhook',
    ...
    if (healthScore() < 60) {
      ...
      alerts.unshift({
        id: `a-${Date.now()}`,
        type: 'health_warning',
        message: `Health ${healthScore()}/100 (${healthGrade()}). Run: node sentinel.cjs --heal`,
        ts: Date.now(),
        acknowledged: false,
      })
    ...
    try {
      ...
      await fetch(body.webhookUrl, {
        ...
        body: JSON.stringify({
          embeds: [
            {
              title: `PCP ${(level || 'info').toUpperCase()}`,
              description: message,
              color,
              footer: { text: `${AGENT_NAME} v${VERSION} (PCP ${PCP_VERSION})` },
...
router.get('/telemetry', async (c) => {
  ...
  const records = await kvGet(kv, 'telemetry', null)
...
router.get('/task', async (c) => {
  ...
  url.pathname = url.pathname.replace('/task', '/tasks')
...
router.post('/task', async (c) => {
  ...
  if (action === 'clear_completed') {
    const remaining = tasks.filter((t: any) => t.status !== 'completed')
    await kvSet(kv, 'tasks', remaining)
    return c.json({ ok: true, cleared: tasks.length - remaining.length })
  }
...
router.post('/act', async (c) => {
  ...
  if (!command)
    return c.json({
      agent: AGENT_NAME,
      available: Object.entries(COMMANDS).map(([name, cmd]) => ({ name, desc: cmd.desc, run: cmd.run })),
    })
  const cmd = COMMANDS[command]
  if (!cmd) return c.json({ ok: false, error: `Unknown command: ${command}`, available: Object.keys(COMMANDS) }, 400)
  await appendLog(kv, { type: 'action_requested', command, by: body.agent || 'unknown' })
...
router.post('/webhook/github', async (c) => {
  ...
  const url = new URL(c.req.url)
  url.pathname = url.pathname.replace('/webhook/github', '/webhooks/github')
