...
router.post('/chat', async (c) => {
  try {
    ...
    // Call AI
    const response = await aiChat({ message, conversationHistory, language, currentPage, userContext }, apiKey, baseUrl)
...
router.post('/stream', async (c) => {
  try {
    ...
    if (!apiKey) {
      ...
      return new Response(`data: ${JSON.stringify({ content: fallbackMessage, done: true })}\n\n`, {
        headers: {
    ...
    if (!stream) {
      ...
      return new Response(`data: ${JSON.stringify({ content: errorMessage, done: true })}\n\n`, {
        headers: {
          'Content-Type': 'text/event-stream',
          'Cache-Control': 'no-cache',
        },
          ...
          while (true) {
            ...
            if (done) {
              ...
              if (actions && actions.length > 0) {
                controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify({ actions, done: false })}\n\n`))
    ...
    return new Response(`data: ${JSON.stringify({ error: String(e), done: true })}\n\n`, {
      headers: {
        'Content-Type': 'text/event-stream',
        'Cache-Control': 'no-cache',
      },
