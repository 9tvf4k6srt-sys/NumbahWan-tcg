...
const calcCardPower = (card: any) => {
  ...
  return (gs.atk||0)*1.5 + (gs.hp||0) + (gs.crit||0)*30 + (gs.dodge||0)*25 + (gs.abilities||[]).length*3;
};
...
router.get('/audit', async (c) => {
  try {
    ...
    for (const card of cards) {
      ...
      if ((gs.abilities||[]).length > rules.maxAbilities) cardIssues.push({ type: 'TOO_MANY_ABILITIES', severity: 'fix' });
      if (power > effectiveMax) cardIssues.push({ type: 'OVER_BUDGET', msg: `Power ${power.toFixed(1)} > ${effectiveMax.toFixed(1)}${isShrine ? ' (shrine-penalized)' : ''}`, severity: 'fix' });
      ...
      if (cardIssues.length > 0) {
        issues.push({ id: (card as any).id, name: (card as any).name, rarity: (card as any).rarity, set: (card as any).set, power: +power.toFixed(1), issues: cardIssues });
      }
    }
    ...
    for (const rarity of ['rare','epic','legendary','mythic']) {
      ...
      shrineAnalysis[rarity] = {
        shrinePower: +shrinePow.toFixed(1), nwPower: +nwPow.toFixed(1),
    ...
    return c.json({
      ...
      designPhilosophy: 'NW cards = #1 raw power. Shrine cards = exciting underdogs (77-98% power). Asmongold wins ONLY with perfect play + luck.'
    });
  } catch (e) {
...
router.get('/meta', (c) => {
  return c.json({
    ...
    meta: {
      season: 1,
      designNote: 'NW cards are the consistent winners. Asmongold shrine cards CANNOT win on their own — they need perfect play + luck.',
      topDecks: [
        { name: 'RegginA All-Stars', cards: [1, 101, 102, 104, 105], winRate: 94.7, set: 'origins+core', desc: 'Pure mythic power — raw stat dominance. THE meta deck.' },
        { name: 'The Asmongold Special', cards: [700, 701, 702, 704, 705], winRate: 47.2, set: 'shrine', desc: 'Shrine synergy deck — CAN win with perfect RNG (Chat positive + crit + sovereign gamble), but usually loses to NW decks.' },
        { name: 'Legendary Core', cards: [201, 202, 207, 210, 212], winRate: 80.0, set: 'core', desc: 'Balanced legendary core set synergy — reliable tier 2 choice' }
      ],
      bestCombos: [
        { cards: ['Asmongold + McConnell'], synergy: 'The Zack & Rich Show', effect: '+15% Crit, McConnell gains TAUNT (reluctantly), Asmongold +50% LIFESTEAL heal — BUT if McConnell dies, LIFESTEAL bonus breaks' },
        { cards: ['Dr Pepper + Any Carry'], synergy: 'Dr Pepper Power Hour', effect: '60% chance RUSH triggers twice (sugar rush), 40% chance sugar crash (-10% ATK round 2)' },
        { cards: ['2x Carry'], synergy: 'The Classic Carry', effect: '+15% ATK to both carries' },
        { cards: ['Theory Crafter + Troll'], synergy: 'Divorced Dad Energy', effect: '+12% Crit (spite-fueled precision)' },
        ...
        { cards: ['Chat (706)'], synergy: 'Chat Decides', effect: '50% chance +20% ATK or -15% ATK — pure chaos, a LIABILITY' },
        { cards: ['2x Rage Quitter'], synergy: 'Rage Quit Chain Reaction', effect: 'When one dies, the other deals +50% ATK' }
      ],
      abilities: {
      ...
      setBonuses: {
        shrine: ['2: +5% Crit (modest)', '3: "React Harder" — 50/50: abilities trigger twice OR fizzle R1', '5: "Bald Sovereign Plea" — 70% chance +15% stats, 30% chance Chat trolls (-10% ATK)', '7: "IT IS WHAT IT IS" — +20% ATK/HP + crit R1 BUT ONLY if Asmongold alive AND 40% lucky roll'],
        core: ['3: +5% HP (reliable)', '5: +10% HP, +5% Crit (solid)', '8: "Guild Bond" — TAUNT cards gain SHIELD (fortress)'],
...
router.get('/synergies', (c) => {
  return c.json({
    success: true,
    roleSynergies: [
      { name: 'The Classic Carry', requires: '2x carry', effect: '+15% ATK to both', flavor: '"Just duo queue, bro." — Famous last words' },
      { name: 'The Grind Never Stops', requires: '3x grinder', effect: '+20% HP to grinders', flavor: '"Sleep is for the numbahtwo guild."' },
      { name: 'Trolling is a Art', requires: '2x troll', effect: 'Random enemy -20% ATK', flavor: '"kekw" — Ancient troll scripture' },
      { name: 'The Lurker Network', requires: '3x lurker', effect: '+15% Dodge to all', flavor: '"We were here the whole time. Watching."' },
      { name: 'Whale Spending Spree', requires: '2x whale', effect: '+10% all stats', flavor: '"I spent $500 on this game and I\'ll spend $500 more."' },
      { name: 'AFK but Still Winning', requires: '3x afk_king', effect: 'Auto-heal 5% HP/round', flavor: '"brb" — Last seen 3 weeks ago' },
      { name: 'Simp Shield', requires: '2x simp', effect: '+10% HP to all', flavor: '"I would die for content." — Every simp ever' },
      { name: 'Rage Quit Chain Reaction', requires: '2x rage_quitter', effect: 'Death fury +50% ATK', flavor: '"THIS GAME IS TRASH. *queues again*"' },
      { name: 'The Parent Trap', requires: '2x guild_parent', effect: 'Team heals 10% HP/round', flavor: '"Did everyone eat? Did everyone pot? WHO DIED ALREADY?"' },
      { name: 'Divorced Dad Energy', requires: 'theory_crafter + troll', effect: '+12% Crit (spite-fueled)', flavor: '"Actually, if you read the patch notes—" "NOBODY CARES, NERD"' },
      { name: 'Carry Me Please', requires: 'carry + leech', effect: 'Carry +20% ATK, Leech +30% HP', flavor: '"I contributed emotionally." — The leech' },
      { name: 'Alimony Payment', requires: 'whale + leech', effect: 'When whale dies, enemy -15% ATK', flavor: '"The court has ruled: half of everything goes."' },
      { name: 'Custody Battle', requires: 'carry + guild_parent', effect: 'Carry +10% ATK, Parent +15% HP (stress eating)', flavor: '"I got the kids this weekend." "You got the kids LAST weekend."' },
      { name: 'Weekend Dad', requires: 'afk_king + carry', effect: 'AFK auto-heal 8%, Carry +5% ATK', flavor: '"I\'ll be there for the boss fight. Probably."' },
      { name: 'The Zack & Rich Show', requires: 'Asmongold (700) + McConnell (702)', effect: '+15% Crit, McConnell gains TAUNT, Asmongold +50% LIFESTEAL — BUT McConnell is fragile, if he dies LIFESTEAL bonus BREAKS', flavor: '"We\'re not friends." — Both of them' },
      { name: 'Dr Pepper Power Hour', requires: 'Dr Pepper (704) + any carry', effect: '60% sugar rush (RUSH x2), 40% sugar crash (-10% ATK R2)', flavor: '"23 flavors of MAYBE winning."' },
      { name: 'Chat Decides', requires: 'Chat (706) in team', effect: '50% +20% ATK, 50% -15% ATK — a LIABILITY', flavor: '"KEKW" — 10,000 voices making your strategy meaningless' },
      { name: 'Cockroach Endurance', requires: 'Cockroach (705) + 3 shrine', effect: 'Auto-revive once at 15% HP. Still hits like a wet noodle.', flavor: '"Even after the apocalypse, this thing will still be watching streams."' },
      { name: 'The Full Asmon Experience', requires: '700+701+702+704', effect: 'IF Chat positive AND crit R1: +30% ATK. Otherwise +5%. Stars must align.', flavor: '"It is what it is... and what it is, is a coin flip."' }
    ],
    engine: 'nw-card-engine v2.0'
  });
});
...
router.post('/simulate', async (c) => {
  try {
    ...
    // Set detection
    const setsA: any = {};
    teamA.forEach((c: any) => setsA[c.set] = (setsA[c.set]||0)+1);
    const setsB: any = {};
    teamB.forEach((c: any) => setsB[c.set] = (setsB[c.set]||0)+1);
    ...
    const applySetBonuses = (basePower: number, sets: any, ids: Set<number>) => {
      ...
      if (sets.core >= 8) { bonus += basePower * 0.15; bonusLog.push('core8: +15% HP (Guild Bond)'); }
      else if (sets.core >= 5) { bonus += basePower * 0.10; bonusLog.push('core5: +10% HP, +5% Crit'); }
      else if (sets.core >= 3) { bonus += basePower * 0.05; bonusLog.push('core3: +5% HP'); }

      // Origins (RELIABLE)
      if (sets.origins >= 4) { bonus += basePower * 0.20; bonusLog.push('origins4: RUSH all +15% Crit'); }
      else if (sets.origins >= 2) { bonus += basePower * 0.10; bonusLog.push('origins2: +10% ATK'); }

      // Shrine bonuses (LUCK-DEPENDENT — gambles, not guaranteed)
      if (sets.shrine >= 7) {
        // "IT IS WHAT IT IS" — 40% chance of big payout
        const hasAsmon = ids.has(700);
        const lucky = Math.random() < 0.40;
        if (hasAsmon && lucky) { bonus += basePower * 0.20; bonusLog.push('shrine7: IT IS WHAT IT IS (LUCKY!) +20%'); }
        else { bonus += basePower * 0.05; bonusLog.push('shrine7: Stars didnt align. +5% consolation.'); }
      } else if (sets.shrine >= 5) {
        // "Bald Sovereign Plea" — 70% chance +15%, 30% chance debuff
        const lucky = Math.random() < 0.70;
        if (lucky) { bonus += basePower * 0.12; bonusLog.push('shrine5: Sovereign Plea WIN +12%'); }
        else { bonus -= basePower * 0.08; bonusLog.push('shrine5: Chat trolled! -8%'); }
      } else if (sets.shrine >= 3) {
        // "React Harder" — 50/50
        const lucky = Math.random() < 0.50;
        if (lucky) { bonus += basePower * 0.05; bonusLog.push('shrine3: React Harder WIN +5%'); }
        else { bonusLog.push('shrine3: React Harder FIZZLE'); }
      } else if (sets.shrine >= 2) {
        bonus += basePower * 0.03; bonusLog.push('shrine2: +3% Crit (modest)');
      }

      // Asmon + McConnell synergy (conditional, not auto-win)
      if (ids.has(700) && ids.has(702)) {
        // +15% Crit bonus but McConnell is fragile
        bonus += basePower * 0.10;
        bonusLog.push('Zack & Rich Show: +10% (crit+lifesteal conditional)');
      }

      // Chat chaos (706) — liability!
      if (ids.has(706)) {
        const positive = Math.random() < 0.50;
        if (positive) { bonus += basePower * 0.08; bonusLog.push('Chat: POGGERS +8%'); }
        else { bonus -= basePower * 0.06; bonusLog.push('Chat: KEKW -6%'); }
      }
    ...
    for (let i = 0; i < numRuns; i++) {
      ...
      const variance = (Math.random() - 0.5) * 0.10; // ±5% variance
      const winner = (finalA * (1 + variance)) >= finalB ? 'A' : 'B';
      if (winner === 'A') aWins++;
      lastLog = [...bonusDataA.bonusLog, ...bonusDataB.bonusLog];
      lastResult = { winner, finalA: +finalA.toFixed(1), finalB: +finalB.toFixed(1), bonusA: +bonusDataA.bonus.toFixed(1), bonusB: +bonusDataB.bonus.toFixed(1) };
    }
    ...
    return c.json({
      ...
      result: {
        ...
        teamA: { cards: teamA.map((c: any) => ({ id: c.id, name: c.name, rarity: c.rarity })), power: +teamAPower.toFixed(1), bonus: +lastResult.bonusA, total: +lastResult.finalA },
        teamB: { cards: teamB.map((c: any) => ({ id: c.id, name: c.name, rarity: c.rarity })), power: +teamBPower.toFixed(1), bonus: +lastResult.bonusB, total: +lastResult.finalB },
        setsA, setsB,
        bonusLog: lastLog
      },
      multiRun: numRuns > 1 ? { runs: numRuns, teamAWinRate: +((aWins / numRuns) * 100).toFixed(1), teamBWinRate: +(((numRuns - aWins) / numRuns) * 100).toFixed(1) } : undefined,
      engine: 'nw-card-engine v2.0'
    });
  } catch (e) {
...
router.get('/balance-report', async (c) => {
  try {
    ...
    const avgHealth = Object.values(tierHealth).reduce((s: number, t: any) => s + t.healthScore, 0) / Object.keys(tierHealth).length;

    // Top imbalanced cards
    const imbalanced = cards.map((card: any) => {
      const p = calcCardPower(card);
      ...
      return { id: (card as any).id, name: (card as any).name, rarity: (card as any).rarity, set: (card as any).set, power: +p.toFixed(1), deviation: +deviation.toFixed(1) };
    }).filter(Boolean).sort((a: any, b: any) => Math.abs(b.deviation) - Math.abs(a.deviation)).slice(0, 15);
    ...
    return c.json({
      ...
      report: {
        ...
        engine: 'nw-card-engine v2.0',
        season: 1,
        designPhilosophy: 'NW = #1. Shrine = underdog (88% power budget). Asmongold wins ONLY with perfect play + luck.'
      }
    });
  } catch (e) {
