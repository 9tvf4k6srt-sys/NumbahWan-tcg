...
function generateTxHash(): string {
  return `TX-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).substring(2, 10).toUpperCase()}`
}
...
router.post('/register', async (c) => {
  try {
    ...
    await db
      ...
      VALUES (?, ?)
    `)
      .bind(citizenId, walletAddress)
      .run()

    // Get wallet info
    const wallet = (await db.prepare('SELECT * FROM wallets WHERE citizen_id = ?').bind(citizenId).first()) as Record<
      string,
      any
    > | null
    ...
    `)
      .bind(txHash, wallet.id, citizenId, deviceUUID)
      .run()
    ...
    `)
      .bind(generateTxHash(), wallet.id, citizenId, deviceUUID)
      .run()
    ...
    return c.json({
      ...
      bonuses: [
        { currency: 'NWG', amount: 100.0, reason: 'Welcome bonus' },
...
router.post('/transaction', async (c) => {
  try {
    ...
    if (isSpend) {
      ...
      `)
        .bind(newBalance, Math.abs(amount), citizen.wallet_id)
        .run()
    } else {
      await db
        .prepare(`
...
router.get('/transactions/:deviceUUID', async (c) => {
  try {
    ...
    const citizen = (await db
      .prepare('SELECT id FROM citizens WHERE device_uuid = ?')
      .bind(deviceUUID)
      .first()) as Record<string, any> | null
