...
router.get('/shrine-meta', async (c) => {
  try {
    ...
    const shrineDetail = shrineCards.map((card: any) => ({
      ...
      image: card.image || `/static/images/cards/shrine-${card.name.toLowerCase().replace(/[^a-z]/g, '-').replace(/-+/g, '-')}.webp`
    }));
    ...
    return c.json({
      ...
      shrineMeta: {
        ...
        designNote: 'Shrine cards are UNDERDOGS (77-98% of NW peer power). They CANNOT auto-win. Every bonus is gamble-dependent. Asmongold wins ONLY with flawless play + luck.',
        cards: shrineDetail,
        setBonuses: [
          ...
          { count: 7, bonus: '"IT IS WHAT IT IS" — +20% ATK/HP + crit R1 BUT ONLY if Asmongold alive AND 40% lucky roll (otherwise +5% consolation)' }
        ],
        uniqueSynergies: [
          { name: 'The Zack & Rich Show', cards: [700, 702], effect: '+15% Crit, McConnell gains TAUNT (reluctantly), Asmongold +50% LIFESTEAL heal — BUT if McConnell dies early, LIFESTEAL bonus BREAKS' },
          { name: 'Dr Pepper Power Hour', cards: [704], effect: '60% chance RUSH triggers twice (sugar rush), 40% chance sugar crash (-10% ATK round 2)' },
          { name: 'Chat Decides', cards: [706], effect: '50% chance +20% ATK, 50% chance -15% ATK — pure chaos, honestly a liability' },
          { name: 'Cockroach Endurance', cards: [705], effect: 'With 3+ shrine cards: auto-revive once at 15% HP. Still hits like a wet noodle.' },
          { name: 'The Full Asmon Experience', cards: [700, 701, 702, 704], effect: 'IF Chat chaos positive AND crit procs R1: +30% ATK. Otherwise just +5%. The stars must align.' }
        ],
        bestDeck: {
          name: 'The Asmongold Special',
          cards: [700, 701, 702, 704, 705],
          estimatedWinRate: 47.2,
          strategy: 'Stack shrine set bonuses + pray for good RNG. Needs Chat positive, Sovereign gamble, AND crit luck to compete with NW core. Fun as hell when it works, heartbreaking when it doesn\'t.'
        }
      }
    });
  } catch (e) {
...
router.post('/rebalance', async (c) => {
  try {
    ...
    for (const card of cards) {
      ...
      if ((gs.abilities||[]).length > rules.maxAbilities) cardFixes.push({ field: 'abilities', from: gs.abilities, to: gs.abilities.slice(0, rules.maxAbilities) });

      if (power > effectiveMax) {
        const excess = power - effectiveMax * 0.80;
        if (excess > 0) {
      ...
      if (cardFixes.length > 0) {
        fixes.push({ cardId: (card as any).id, cardName: (card as any).name, rarity: (card as any).rarity, set: (card as any).set, changes: cardFixes });
      }
    }
...
router.post('/test-deck', async (c) => {
  try {
    ...
    for (let i = 0; i < numSims; i++) {
      ...
      const vA = (Math.random() - 0.5) * 0.10;
      const vB = (Math.random() - 0.5) * 0.10;
