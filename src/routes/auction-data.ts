/**
 * Auction House Routes - Live Card Trading System
 * NEW FEATURE: Real-time bidding, price history, notifications
 * 
 * AUTO-LEARN PATTERN: auction-house-system
 * Type: feature
 * Impact: Major new economy feature - drives engagement
 * 
 * Features:
 * - Time-limited auctions (1h, 6h, 24h, 72h)
 * - Real-time bidding with minimum increment
 * - Bid sniping protection (auto-extend timer)
...
export function updateTraderStats(wallet: string, name: string, type: 'buy' | 'sell', amount: number) {
  const stats = traderStats.get(wallet) || {
    ...
    winRate: 0, avgBidAmount: 0,
    favRarity: 'common', reputation: 100
  }
  if (type === 'buy') {
    stats.totalBought++
...
export function initSampleAuctions() {
  ...
  const samples = [
    { card: { id: 1, name: 'RegginA, The Eternal Flame', rarity: 'mythic', img: '01-reggina-mythic-maplestory.webp' }, startPrice: 500, buyNow: 2000, seller: 'GuildVault', sellerId: 'npc-vault' },
    { card: { id: 27, name: 'Elder Dragon', rarity: 'legendary', img: '27-elder-dragon.webp' }, startPrice: 100, buyNow: 500, seller: 'DragonKeeper', sellerId: 'npc-dragon' },
    { card: { id: 3, name: 'Onca, The Jungle Fury', rarity: 'epic', img: 'epic-onca.webp' }, startPrice: 30, buyNow: 150, seller: 'CardMaster', sellerId: 'npc-cards' },
    { card: { id: 2, name: 'Reggino, Vice Commander', rarity: 'legendary', img: '02-reggino-legendary.webp', stars: 3 }, startPrice: 200, buyNow: 800, seller: 'Collector42', sellerId: 'npc-collector' },
    { card: { id: 5, name: 'Panthera, Shadow Stalker', rarity: 'epic', img: 'epic-panthera.webp' }, startPrice: 15, seller: 'NewTrader', sellerId: 'npc-new' },
  ]
