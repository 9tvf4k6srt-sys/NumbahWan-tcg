/**
 * NumbahWan TCG - Season Card Generator
 * Generates card JSON files for all 10 seasons with fun, compelling content
 */
...
// ============================================
// SEASON DEFINITIONS - The fun stuff!
// ============================================
...
const SEASONS = {
  3: {
    ...
    mechanics: {
      HACK: { name: "System Breach", desc: "Take control of enemy card for 1 turn", color: "#00ffff" },
      FIREWALL: { name: "Digital Shield", desc: "Block the next attack completely", color: "#0080ff" },
      OVERCLOCK: { name: "Turbo Mode", desc: "Double ATK but take damage after", color: "#ff0080" },
      VIRUS: { name: "Malware", desc: "Spread -1/-1 to adjacent enemies", color: "#00ff00" },
      REBOOT: { name: "Respawn.exe", desc: "Return to hand when destroyed", color: "#ffff00" }
    },
    categories: {
      virus: { icon: "🦠", name: "Virus" },
      program: { icon: "💾", name: "Program" },
      ai: { icon: "🤖", name: "AI" },
      hacker: { icon: "👨‍💻", name: "Hacker" },
      firewall: { icon: "🛡️", name: "Firewall" },
      glitch: { icon: "⚡", name: "Glitch" }
    },
    // Card templates - name, rarity, category, description, abilities, special
    cards: [
      // MYTHICS (2)
      ...
      { name: { en: "Root Access", zh: "根权限", th: "Root Access" }, rarity: "legendary", category: "hacker", desc: { en: "sudo rm -rf * ...wait, that's not what I meant!", zh: "sudo rm -rf * ...等等，这不是我想的！", th: "sudo rm -rf * ...เดี๋ยว นั่นไม่ใช่ที่ฉันหมายถึง!" }, abilities: ["HACK", "OVERCLOCK"], special: { name: "SUDO", desc: "Bypass all permissions" } },
      ...
      { name: { en: "Meme Lord Protocol", zh: "梗王协议", th: "โปรโตคอลมีมลอร์ด" }, rarity: "epic", category: "ai", desc: { en: "Achieved meme consciousness. Communicates in reaction images.", zh: "达成迷因意识。用表情包交流。", th: "บรรลุจิตสำนึกมีม สื่อสารด้วยรูปรีแอคชัน" }, abilities: ["HACK", "VIRUS"], special: { name: "VIRAL MEME", desc: "Summon a copy when this wins" } },
      ...
      { name: { en: "Keyboard Warrior", zh: "键盘侠", th: "นักรบคีย์บอร์ด" }, rarity: "rare", category: "hacker", desc: { en: "Feared in forums. Useless IRL.", zh: "论坛里可怕。现实中没用。", th: "น่ากลัวในฟอรัม ไร้ประโยชน์ในชีวิตจริง" }, abilities: ["OVERCLOCK"], special: { name: "CAPS LOCK", desc: "+3 ATK but enemy's next attacker also +3" } },
      ...
      { name: { en: "System32 Guardian", zh: "System32守护者", th: "System32 Guardian" }, rarity: "rare", category: "firewall", desc: { en: "DELETE ME NOT. I AM IMPORTANT.", zh: "别删我。我很重要。", th: "อย่าลบฉัน ฉันสำคัญ" }, abilities: ["FIREWALL"], special: { name: "CRITICAL SYSTEM", desc: "If destroyed, lose 2 random deck cards" } },
      { name: { en: "End Process", zh: "结束进程", th: "End Process" }, rarity: "rare", category: "program", desc: { en: "Task Manager's nuclear option.", zh: "任务管理器的核选项。", th: "ตัวเลือกนิวเคลียร์ของ Task Manager" }, abilities: ["OVERCLOCK"], special: { name: "FORCE QUIT", desc: "Destroy any card with less HP than this ATK" } },
  ...
  4: {
    ...
    mechanics: {
      LURK: { name: "Hidden Terror", desc: "Hidden until you choose to reveal", color: "#4a0080" },
      SHADOW: { name: "Darkness Cloak", desc: "Cannot be targeted by spells", color: "#1a0033" },
      VOID: { name: "Oblivion", desc: "Banish instead of destroy", color: "#000000" },
      NIGHTMARE: { name: "Dream Eater", desc: "Enemy skips next attack", color: "#8b0080" },
      STEALTH: { name: "Unseen", desc: "Ignore GUARD effects", color: "#2d1b4e" }
    },
    categories: {
      shadow: { icon: "👤", name: "Shadow" },
      nightmare: { icon: "👁️", name: "Nightmare" },
      void: { icon: "🕳️", name: "Void" },
      undead: { icon: "💀", name: "Undead" },
      phantom: { icon: "👻", name: "Phantom" },
      demon: { icon: "😈", name: "Demon" }
    },
    cards: [
      // MYTHICS
      ...
      { name: { en: "Jumpcare Jack", zh: "惊吓杰克", th: "Jack Jumpscare" }, rarity: "epic", category: "nightmare", desc: { en: "BOO! ...Did I get you? The terror never fades.", zh: "嘘！...吓到你了吗？恐惧永不消退。", th: "บู้! ...ตกใจไหม? ความหวาดกลัวไม่เคยจางหาย" }, abilities: ["STEALTH"], special: { name: "JUMPSCARE", desc: "Deal triple damage on first attack" } },
      ...
      { name: { en: "The Forgotten One", zh: "被遗忘者", th: "ผู้ถูกลืม" }, rarity: "epic", category: "void", desc: { en: "Once had a name. Now has nothing.", zh: "曾经有名字。现在什么都没有。", th: "เคยมีชื่อ ตอนนี้ไม่มีอะไร" }, abilities: ["VOID", "STEALTH"], special: { name: "FORGOTTEN", desc: "Cannot be targeted or remembered in any way" } },
      
      // RARES (15) - Keeping abbreviated for space
      ...
      // UNCOMMONS (15)
      { name: { en: "Creaking Door", zh: "吱呀作响的门", th: "ประตูดังเอี๊ยด" }, rarity: "uncommon", category: "shadow", desc: { en: "Nobody opened it. It opened anyway.", zh: "没人打开它。它还是开了。", th: "ไม่มีใครเปิด มันเปิดเอง" }, abilities: ["LURK"], special: { name: "CREAK", desc: "Reveal hidden card" } },
      { name: { en: "Flickering Light", zh: "闪烁的灯", th: "แสงกะพริบ" }, rarity: "uncommon", category: "phantom", desc: { en: "The light bulb is fine. Then why is it flickering?", zh: "灯泡没问题。那为什么在闪？", th: "หลอดไฟปกติ แล้วทำไมมันกะพริบ?" }, abilities: ["NIGHTMARE"], special: { name: "FLICKER", desc: "Blind enemy for 1 turn" } },
      { name: { en: "Cold Spot", zh: "冷点", th: "จุดเย็น" }, rarity: "uncommon", category: "phantom", desc: { en: "Suddenly cold. Breath visible. Something's here.", zh: "突然很冷。能看到呼吸。有什么在这。", th: "เย็นทันที เห็นลมหายใจ มีบางอย่างอยู่ที่นี่" }, abilities: ["STEALTH"], special: { name: "CHILL", desc: "Slow enemy by 3" } },
      { name: { en: "Shadow Puppet", zh: "皮影戏", th: "หุ่นเงา" }, rarity: "uncommon", category: "shadow", desc: { en: "Cute on the wall. Less cute when it moves alone.", zh: "在墙上很可爱。自己动时就不可爱了。", th: "น่ารักบนผนัง น่ารักน้อยลงเมื่อขยับเอง" }, abilities: ["LURK"], special: { name: "PUPPET", desc: "Control enemy card's attack target" } },
      { name: { en: "Dust Bunny Demon", zh: "灰尘兔恶魔", th: "ปีศาจกระต่ายฝุ่น" }, rarity: "uncommon", category: "demon", desc: { en: "Lives under your bed. Has grown sentient. Hungry.", zh: "住在你床下。已经有了意识。饿了。", th: "อาศัยใต้เตียง มีความคิดแล้ว หิว" }, abilities: ["LURK"], special: { name: "DUSTY", desc: "Reduce enemy ATK by 2" } },
      { name: { en: "Uncanny Valley Resident", zh: "恐怖谷居民", th: "ผู้อยู่อาศัย Uncanny Valley" }, rarity: "uncommon", category: "nightmare", desc: { en: "Almost looks human. Almost. Not quite.", zh: "看起来几乎像人。几乎。不完全是。", th: "ดูเกือบเป็นมนุษย์ เกือบ ไม่ใช่ซะทีเดียว" }, abilities: ["NIGHTMARE"], special: { name: "UNSETTLING", desc: "Enemy skips attack if below 5 HP" } },
      { name: { en: "Abandoned Doll", zh: "被遗弃的娃娃", th: "ตุ๊กตาที่ถูกทอดทิ้ง" }, rarity: "uncommon", category: "shadow", desc: { en: "Its eyes follow you. Even when you're not looking.", zh: "它的眼睛跟着你。即使你不看它时也是。", th: "ตามองตามคุณ แม้คุณไม่มอง" }, abilities: ["LURK"], special: { name: "STARE", desc: "Enemy cannot heal" } },
      { name: { en: "Midnight Snack", zh: "午夜零食", th: "ของว่างเที่ยงคืน" }, rarity: "uncommon", category: "demon", desc: { en: "You're the snack.", zh: "你就是零食。", th: "คุณคือของว่าง" }, abilities: ["STEALTH"], special: { name: "MUNCH", desc: "Heal 3 HP when dealing damage" } },
      { name: { en: "Déjà Vu", zh: "既视感", th: "Déjà Vu" }, rarity: "uncommon", category: "void", desc: { en: "Haven't we done this before? Again? And again?", zh: "我们之前做过这个吗？又来？又来？", th: "เราเคยทำแบบนี้ไหม? อีกแล้ว? อีกแล้ว?" }, abilities: ["NIGHTMARE"], special: { name: "REPEAT", desc: "Play the same card twice" } },
      { name: { en: "Imposter Syndrome", zh: "冒充者综合征", th: "Imposter Syndrome" }, rarity: "uncommon", category: "phantom", desc: { en: "They'll find out you're a fraud. Any moment now.", zh: "他们会发现你是骗子。随时都会。", th: "พวกเขาจะพบว่าคุณเป็นคนหลอกลวง อีกสักครู่" }, abilities: ["NIGHTMARE"], special: { name: "FRAUD", desc: "Your cards appear as random cards to enemy" } },
      { name: { en: "Void Puppy", zh: "虚空小狗", th: "ลูกสุนัขว่างเปล่า" }, rarity: "uncommon", category: "void", desc: { en: "Good boy! ...Where did it go? Was it ever there?", zh: "好孩子！...它去哪了？它存在过吗？", th: "น้องหมาดี! ...มันไปไหน? มันเคยอยู่ไหม?" }, abilities: ["VOID"], special: { name: "BLINK", desc: "Teleport to different position" } },
      { name: { en: "Ominous Dripping", zh: "不祥的滴水声", th: "เสียงหยดน้ำชวนขนลุก" }, rarity: "uncommon", category: "shadow", desc: { en: "Drip. Drip. Drip. What is it? Don't look.", zh: "滴。滴。滴。是什么？别看。", th: "หยด หยด หยด มันคืออะไร? อย่ามอง" }, abilities: ["NIGHTMARE"], special: { name: "DRIP", desc: "Deal 1 damage per turn passively" } },
      { name: { en: "Wrong Turn Spirit", zh: "迷路幽灵", th: "วิญญาณเลี้ยวผิด" }, rarity: "uncommon", category: "phantom", desc: { en: "You shouldn't have come this way.", zh: "你不应该走这边来。", th: "คุณไม่ควรมาทางนี้" }, abilities: ["LURK"], special: { name: "LOST", desc: "Shuffle enemy's deck" } },
      { name: { en: "Procrastination Demon", zh: "拖延恶魔", th: "ปีศาจผัดวันประกันพรุ่ง" }, rarity: "uncommon", category: "demon", desc: { en: "You'll deal with it later. Much later. Never.", zh: "你以后再处理。很久以后。永远不。", th: "คุณจะจัดการทีหลัง ทีหลังมาก ไม่เคย" }, abilities: ["NIGHTMARE"], special: { name: "LATER", desc: "Delay enemy's next action by 1 turn" } },
      { name: { en: "Self-Doubt Specter", zh: "自我怀疑幽灵", th: "ผีความสงสัยในตัวเอง" }, rarity: "uncommon", category: "phantom", desc: { en: "Are you sure that was the right play?", zh: "你确定那是正确的选择吗？", th: "คุณแน่ใจว่านั่นคือการเล่นที่ถูกต้องไหม?" }, abilities: ["NIGHTMARE"], special: { name: "DOUBT", desc: "Enemy's next ability has 50% chance to fail" } },
      
      // COMMONS (20) - Abbreviated
  ...
  // Seasons 5-10 will be added in similar format...
};
...
function generateStats(rarity) {
  ...
  return {
    atk: randomBetween(r.atk[0], r.atk[1]),
    hp: randomBetween(r.hp[0], r.hp[1]),
    cost: randomBetween(r.cost[0], r.cost[1]),
    spd: randomBetween(r.spd[0], r.spd[1]),
    crit: randomBetween(r.crit[0], r.crit[1]),
    dodge: randomBetween(r.dodge[0], r.dodge[1])
  };
}
...
function generateSeasonJSON(seasonNum) {
  ...
  if (!season) {
    console.log(`Season ${seasonNum} not defined yet`);
    return null;
  }
  ...
  const json = {
    ...
    lastUpdated: new Date().toISOString().split('T')[0] + "T00:00:00Z",
    totalCards: season.cards.length,
    lore: season.lore,
    newMechanics: season.mechanics,
    ...
    cards: season.cards.map((card, idx) => ({
      ...
      hasArt: false, // Coming soon placeholder
      gameStats: generateStats(card.rarity),
...
function generateAllSeasons() {
  ...
  for (const seasonNum of Object.keys(SEASONS)) {
    const json = generateSeasonJSON(parseInt(seasonNum));
    if (json) {
      const filename = `cards-s${seasonNum}.json`;
      const filepath = path.join(outputDir, filename);
      fs.writeFileSync(filepath, JSON.stringify(json, null, 2));
      console.log(`✅ Generated ${filename} with ${json.totalCards} cards`);
    }
  }
  
  console.log('\n🎉 All seasons generated!');
}
