...
const NW_CARD_RENDERER = (function() {
    ...
    function renderCard(card, options = {}) {
        ...
        // Parse stats - prioritize flavorStats (new), fallback to gameStats (legacy)
        let stats = card.flavorStats || card.gameStats || card.stats;
        ...
        // Normalize flavorStats to renderer format
        if (stats) {
            if (stats.power !== undefined) stats.atk = stats.power;
            if (stats.endurance !== undefined) stats.def = stats.endurance;
            if (stats.hp !== undefined && stats.def === undefined) stats.def = stats.hp;
        }
